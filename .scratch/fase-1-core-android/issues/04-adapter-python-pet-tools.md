...
- [ ] `esp32-server/upstream/main/xiaozhi-server/plugins_func/functions/pet_tools.py` criado.
- [ ] 12 tools registradas como `ToolType.SYSTEM_CTL` (2 read + 10 write).
- [ ] Cada tool write chama Core via `httpx` e envia Ações não-verbais via `conn.websocket.send()`.
- [ ] Cada tool read chama Core via `httpx` e retorna estado como contexto.
- [ ] `dynamic_context` injeta estado do pet no PromptManager ao abrir conexão.
- [ ] `pet_tools` adicionado à lista de functions no `Intent.function_call.functions` do config.
- [ ] `httpx` adicionado às dependências do xiaozhi-server (se não já presente).
